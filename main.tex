...
\label{sec:hammer}
	...
	provides an expressive HOL to prove theorems 
	...
	from Isabelle's HOL to the SMT solvers'
	FOL, and the reconstruction of proofs found by the SMT solver with inference rules of Isabelle/HOL. 
	...
	\subsection{Translation}
		...
		implies validity of the translated problem.
		HOL is more expressive than FOL. As such, 
		all first-order terms are representable in 
		HOL. Translation of this first-order subset
		of HOL is pretty straightforward. More 
		interestingly, the translation has to deal 
		...
		The translation is an improvement of the 
		translation used by Sledgehammer for 
		superposition provers, because 
		(monomorphic) types are embedded in 
		SMT-solvers so some of the types from 
		Isabelle such as integers and reals are 
		mapped to their corresponding SMT types.
		
		An evaluation on this translation 
		showed that SMT solvers improved and 
		complemented the proof automation from 
		resolution provers on many trivial and 
		non-trivial problems.
		
	\subsection{The Z3 Proof System}
		Z3's proof system is a natural deduction 
		calculus that uses 38 proof rules. Some of 
		these rules are presented in \textcolor{red}{Fig}
		using sequents $\Gamma \models \varphi$ where 
		...
		proof tree consists of axioms from the 
		proof system as leaves, and application of 
		rules as nodes, and since proof trees 
		...
		the sequent $\Pi^\prime \models \bot$ where 
		...
		to the simple rules from \textcolor{red}{Fig},
		...
		\begin{equation*}
			\infer[\texttt{unit-resolution}]
				{\Gamma \cup \bigcup\limits_{j \in J}\Gamma_j \models \bigvee\limits_{i \in I\setminus J} l_i}
				{\Gamma \models \bigvee\limits_{i \in I}l_i & \langle \Gamma_j \models \neg l_j \rangle_{j \in J}}
		\end{equation*}
		...
		\begin{equation*}
			\infer[\texttt{cong}]
				{\bigcup\limits_{j\in J}\Gamma_j \models f(t_1, ..., t_n) \simeq f(t_1^{\prime},...,t_n^{\prime})}
				{\langle \Gamma_j \models t_j \simeq t_j^{\prime} \rangle_{j \in J} & 
		...
		\begin{equation*}
			...
			{\Gamma \models (Q \overline{x}.\varphi_1 [ \overline{x}]) \sim (Q \overline{x}.\varphi_2 [ \overline{x}])}
			{\Gamma \models \varphi_1 [\overline{x}] \sim \varphi_2 [\overline{x}]}
		\end{equation*}
		...
		negations are applied only to variables and 
		the only Boolean operators in it are conjunction 
		and disjunction. Z3 has a set of rules for 
		...
		and rewriting rules. Rewriting is an important 
		part of SMT solvers - this process takes 
	...
	\subsection{Proof Reconstruction}
		...
		the node's conclusion. Once reconstructed, a 
		proof node can be reused by Sledgehammer.
		If $\Pi$ is the set of asserted formulas, 
		$\Pi^\prime$ represents the unsat core returned 
		by Z3 - the minimal subset of $\Pi$ required 
		to prove the problem unsat. After reconstructing
		the root node, Sledgehammer checks the rest of
		the tree to see that only the formulas from 
		$\Pi^\prime$ remain as leaves.
