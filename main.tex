...
\begin{abstract}
	...
	of the spectrum of verifiers 
	\ct{Why re you calling these tools verifiers? Verifiers are widely understood to be tools for software/hardware verification. These are more general tools: theorem provers}.
	 On one hand, 
...
\label{sec:intro}
	...
	(SMT)-solvers and superposition provers. 
	\ct{There is a lot more than just SMT and superposition provers.
	Say that those are two of the most popular classes of ATPs.
	} 
	Although both 
	...
	is unsatisfiable.
	\ct{The kind of hyphen you need here is ---, not - which is used for compound words. In constrast --  is used for numerical ranges.}
	...
	attempt to combine these tools \ct{Which tools?}. We conclude with a 
	...
	SMTCoq. 
	\ct{Here you are talking about two specific tools}
	Both these tools use SMT-solvers to 
	automate proofs in ITPs, but approach the 
	problem very differently . 
	\ct{Here instead you have now switched to the class of tools Sledgehammer belongs to, but without introduction.}
	A hammer helps an ITP 
	prove lemmas by using an ATP to guide its proof search, 
	and Sledgehammer is	the very first of these. 
	\ct{Similar problem here.} 
